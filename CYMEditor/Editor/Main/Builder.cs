...
namespace CYM
{
    public class Builder
    {
        ...
        static void BuildDLCConfigInternal(DLCItem dlcItem)
        {
            ...
            BaseFileUtil.SaveJson(manifestPath, dlcManifest, true);
            #endregion

            #region dlcitem
            if (File.Exists(dlcItemPath)) File.Delete(dlcItemPath);
            BaseFileUtil.SaveJson(dlcItemPath, dlcItem.Config, true);
            #endregion
            ...
            foreach (var bundleData in dlcManifest.Data)
            {
                ...
                foreach (var pathData in bundleData.AssetFullPaths)
                {
                    ...
                    if (DLCConfig.IsInIgnoreConst(fileName))
                        continue;
                    if (!Consts.ContainsKey(newBundleName))
                        Consts.Add(newBundleName, new Dictionary<string, string>());
                    if (Consts[newBundleName].ContainsKey(fileName))
                        continue;
                    Consts[newBundleName].Add(fileName, pathData.FileName);
                }
            }
        ...
        {
            BaseFileUtil.EnsureDirectory(dlcItem.TargetPath);
        ...
        private static void _BuildEXE()
        {
            ...
            GetBuildData(distribution).PostBuilder();
        }
        ...
        #region get
        static BuildData GetBuildData(Distribution type)
        {
            if (type == Distribution.Steam) return new BuildSteamData();
            else return new BuildData();
    ...
    {
        protected BuildConfig BuildConfig => BuildConfig.Ins;
        public virtual void PostBuilder()
        {
            if (BuildConfig.Platform == Platform.Windows64)
            {
                BaseFileUtil.WriteFile(Path.Combine(BuildConfig.DirPath, CopyRight.CopyRightFileName), CopyRight.Notice);
            }
        }
    }
    #endregion
    ...
    public class BuildSteamData : BuildData
    {
        ...
        {
            base.PostBuilder();
            BaseFileUtil.WriteFile(Path.Combine(BuildConfig.DirPath, SysConst.File_SteamAppID), BuildConfig.Ins.SteamAppID.ToString());
        }
    }
    #endregion
}