...
namespace CYM.UI.JumpText
{
    public class JumpText : MonoBehaviour
    {
        ...
        //Combination:
        static Dictionary<string, HashSet<JumpText>> combinationDictionary;
        JumpText myAbsorber;
        JumpText myTarget;
        ...
        static Dictionary<int, HashSet<JumpText>> pools;
        int poolingID;
        bool destroyOnStart;
        bool performRestart;
        ...
        //Pool Related:
        void CopyDefaults(JumpText prefab)
        {
        ...
        /// </summary>
        /// <returns></returns>
        public JumpText CreateNew(float newNumber, Vector3 newPosition)
        {
            JumpText newDN = default;
            ...
            {
                //Get from Pool:
                foreach(JumpText dn in pools[instanceID])
                {
                    newDN = dn;
                    break;
                }
            ...
            else
            {
                ...
                newDN = newGO.GetComponent<JumpText>();
        ...
        public void PrewarmPool()
        {
            if (enablePooling)
            {
                ...
                if(pools == null)
                {
                    pools = new Dictionary<int, HashSet<JumpText>>();
                }

                //Initialize Pool:
                if(!pools.ContainsKey(instanceId))
                {
                    pools.Add(instanceId, new HashSet<JumpText>());
                }

                //Fill Pool:
                HashSet<JumpText> pool = pools[instanceId];
                for(int n = 0; n < poolSize - pool.Count; n++)
                {
                    JumpText dn = CreateNew(123, Vector3.zero);
        ...
        void TryCombination()
        {
            ...
            {
                combinationDictionary = new Dictionary<string, HashSet<JumpText>>();
            }

            //Create HashSet:
            if (!combinationDictionary.ContainsKey(combinationSettings.combinationGroup))
            {
                combinationDictionary.Add(combinationSettings.combinationGroup, new HashSet<JumpText>());
            }
            ...
            if (combinationSettings.absorberType == AbsorberType.OLDEST)
            {
                ...
                JumpText oldestNumber = null;

                foreach(JumpText otherNumber in combinationDictionary[combinationSettings.combinationGroup])
                {
        ...
        public void GetAbsorbed(JumpText otherNumber)
        {
        ...
        public void DestroyNumber()
        {
            ...
            if (enablePooling)
            {
                ...
                {
                    pools.Add(poolingID, new HashSet<JumpText>());
                }
