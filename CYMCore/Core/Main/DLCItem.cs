    ...
    public class DLCItem
    {
        ...
        public void OnPostBuildBundle()
        {
            ...
            for (int i = 0; i < AbsCopyDirectory.Count; ++i)
            {
                ...
                BaseFileUtil.EnsureDirectory(finalTargetPath);
                BaseFileUtil.CopyDir(absPath, finalTargetPath, false, true);
            }
        }
        #endregion
        ...
        public AssetBundle LoadRawBundle(string name)
        {
            ...
            string path = Path.Combine(Application.streamingAssetsPath, Name) + "/" + Name.ToLower() + "_" + name;
        ...
        public DLCManifest GetManifestFile()
        {
            ...
            if (BuildConfig.Ins.IsEditorOrConfigMode)
            {
                reader = BaseFileUtil.LoadJson<DLCManifest>(GetManifest());
            }
            else
            {
        ...
        {
            var temp = BaseFileUtil.GetFiles(path, extend, SearchOption.AllDirectories);
