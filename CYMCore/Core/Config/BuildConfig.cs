...
namespace CYM
{
    public enum BuildWindowShowType
    {
        Build,
        Dlc,
    }
    [CreateAssetMenu(fileName = "BuildConfig", menuName = "Config/Build", order = 1)]
    public sealed class BuildConfig : ScriptableObjectConfig<BuildConfig> 
    {
        #region Inspector
        public BuildWindowShowType ShowType = BuildWindowShowType.Build;
        [HideInInspector]
        ...
        public string DirPath
        {
            get
            {
                if (IsPublic)
                {
                    if (IsTrial)
                    {
                        return Path.Combine(SysConst.Path_Build, Platform.ToString()) + " " + Distribution;//xxxx/Windows_x64 Trail
                    }
                    else
